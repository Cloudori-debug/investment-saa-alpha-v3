# SAA 알파 v3 — (선택) 백업·업데이트 전 점검

> **필수 아님.** 앱을 쓰려면 `투자나침반.bat`만 있으면 됩니다.  
> 이 문서는 포맷·이사·버전 올리기 **전에** 장부/불변을 확인하고 싶을 때만 봅니다.  
...
필수 사용 조건이 아닙니다. 백업·이사 전에만 체크하세요.
...
평소 사용: **`투자나침반.bat` → 실보유 입력.**  
B단계(설치형·신선도 게이트)는 별도. SaaS(C)는 헌장 밖.

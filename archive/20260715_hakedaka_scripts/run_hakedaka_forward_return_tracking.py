#!/usr/bin/env python3
"""Phase 4i — Hakedaka Forward Return & Catalyst Watchlist Tracking (shadow only)."""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Hakedaka forward return tracking (shadow only)")
    ...
    report = run_hakedaka_forward_return_tracking(
        args.data_dir, args.output_dir, as_of=args.as_of or date.today().isoformat(), top_n=args.top_n,
    )
    out = args.output_dir / "hakedaka_phase4i_run.json"
